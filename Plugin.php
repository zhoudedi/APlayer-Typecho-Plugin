 ...
 * 给予开源项目：<a href="https://github.com/DIYgod/APlayer" target="_blank">APlayer</a>二次开发
 * 
 * @package APlayerAtBottom
 * @author 小周
 * @version 1.1.2
 * @link https://github.com/zhoudedi/APlayer-Typecho-Plugin
 */
class APlayerAtBottom_Plugin implements Typecho_Plugin_Interface
{
    ...
    public static function config(Typecho_Widget_Helper_Form $form){
		...
		//从GithubAPI拉取最新内容
      	$arr = json_decode(@file_get_contents('https://api.github.com/repos/zhoudedi/APlayer-Typecho-Plugin/releases/latest'), true);
    	...
    	$id = new Typecho_Widget_Helper_Form_Element_Text('id', null, '6761209918', _t('歌单id'), '这里填写你的 <b>网易云音乐</b> 或 <b>QQ音乐</b> 歌单ID，请不要填写成为UserID');
        $form->addInput($id);
    	...
    	$form->addInput($tencent);*/
        	...
        	// API可用性检测函数
        function isApiAvailable($url, $isCustom = false) {
        // 自定义API始终视为可用
        if ($isCustom) {
            return true;
        }
        ...
        '1' => [
            'name' => 'Shota\'s API',
            'test_url' => 'https://api.9jojo.cn/netease/?type=playlist&id=12345',
            'is_custom' => false
        ],
        '2' => [
            'name' => 'O\'s API',
            'test_url' => 'https://api.ohmyga.cn/netease/?use=1&type=playlist&id=12345',
            'is_custom' => false
        ],
        '3' => [
            'name' => '犬\'s API',
            'test_url' => 'https://api.fczbl.vip/163/?&type=playlist&id=12345',
            'is_custom' => false
        ],
        '4' => [
            'name' => 'Meto API',
            'test_url' => 'https://api.i-meto.com/meting/api?server=netease&type=playlist&id=12345',
            'is_custom' => false
        ]
        ];
        
        // 筛选可用的网易云API选项
        $availableNeteaseOptions = [];
        foreach ($neteaseApis as $key => $apipd) {
        if (isApiAvailable($apipd['test_url'], $apipd['is_custom'])) {
            $availableNeteaseOptions[$key] = $apipd['name'];
        }
        }
        // 确保至少有一个选项可用
        if (empty($availableNeteaseOptions)) {
        $availableNeteaseOptions['0'] = $neteaseApis['0']['name'];
        }
        // 创建网易云音乐表单元素
        $netease = new Typecho_Widget_Helper_Form_Element_Radio('netease',$availableNeteaseOptions,'1','网易云音乐解析服务器选择','您可以自行选择音乐歌单解析服务器（仅显示可用服务器）');
        $form->addInput($netease);
        ...
        '1' => [
            'name' => 'Meto API',
            'test_url' => 'https://api.i-meto.com/meting/api?server=tencent&type=playlist&id=12345',
            'is_custom' => false
        ]
        ];
        
        // 筛选可用的QQ音乐API选项
        $availableTencentOptions = [];
        foreach ($tencentApis as $key => $apipd) {
        if (isApiAvailable($apipd['test_url'], $apipd['is_custom'])) {
            $availableTencentOptions[$key] = $apipd['name'];
        }
        }
        
        // 确保至少有一个选项可用
        if (empty($availableTencentOptions)) {
        $availableTencentOptions['0'] = $tencentApis['0']['name'];
        }
        
        // 创建QQ音乐表单元素
        $tencent = new Typecho_Widget_Helper_Form_Element_Radio('tencent',$availableTencentOptions,'1','QQ音乐解析服务器选择','您可以自行选择音乐歌单解析服务器（仅显示可用服务器）');
        $form->addInput($tencent);
    ...
    public static function header(){
		...
		if (file_exists(__DIR__ .'/settings.json') == false) {
			...
			if ((time() - $decode['last_update']) > $cachetime) {
				...
				if($apid != $oldapi || $server != $oldserver){
					$data['data'] = @file_get_contents($api_out.$id."&rand=".mt_rand(0,999999));
